...
public class CrescendoEvo extends Evolution {
    ...
    public static int branchID(AbstractCard card) {
        ...
        switch (chosenBranch) {
            case 1:
                return Crescendo_Careful;
            case 2:
                return Crescendo_Origin;
            default:
                return 0;
        }
    }
    ...
    public static List<UpgradeBranch> branches(AbstractCard card) {
        return new ArrayList<UpgradeBranch>() {{
            ...
            add(() -> {
                upgradeEvolvedTexts(card, Crescendo_Careful);
                setMagic(card, 3);
            });
            add(() -> {
                upgradeEvolvedTexts(card, Crescendo_Origin);
                card.isInnate = true;
                card.retain = false;
            });
        }};
    ...
    public static class Evolve {
        ...
        public static void Raw(CtBehavior ctMethodToPatch) throws Exception {
            if (!manipulateMethods(CrescendoEvo.class, ctMethodToPatch))
                Nature.Log("Crescendo is not evolvable.");
        }
    }
