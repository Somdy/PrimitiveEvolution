...
public class CrushJointsEvo extends Evolution {
    ...
    public static int branchID(AbstractCard card) {
        ...
        switch (chosenBranch) {
            case 1:
                return CrushJoints_Vary;
            case 2:
                return CrushJoints_Cripple;
            default:
                return 0;
        }
    }
    ...
    public static List<UpgradeBranch> branches(AbstractCard card) {
        return new ArrayList<UpgradeBranch>() {{
            ...
            add(() -> {
                upgradeEvolvedTexts(card, CrushJoints_Vary);
                setDamage(card, 4);
                upgradeMagic(card, 2);
            });
            add(() -> {
                upgradeEvolvedTexts(card, CrushJoints_Cripple);
                setDamage(card, 6);
                setMagic(card, 1);
            });
        }};
    ...
    @SpirePatch(clz = CrushJoints.class, method = SpirePatch.CONSTRUCTOR)
    public static class Evolve {
        @SpireRawPatch
        public static void Raw(CtBehavior ctMethodToPatch) throws Exception {
            if (!manipulateMethods(CrushJointsEvo.class, ctMethodToPatch))
                Nature.Log("CrushJoints is not evolvable.");
        }
    }
    ...
    @SpirePatch(clz = CrushJoints.class, method = "use")
    public static class Use {
        @SpirePrefixPatch
        public static SpireReturn Prefix(AbstractCard _inst, AbstractPlayer p, AbstractMonster m) {
            if (_inst instanceof EvolvableCard && _inst.upgraded) {
                    switch (((EvolvableCard) _inst).evolanch()) {
                        case CrushJoints_Vary:
                            addToBot(new DamageAction(m, new DamageInfo(p, _inst.damage, _inst.damageTypeForTurn),
                                    AbstractGameAction.AttackEffect.BLUNT_HEAVY));
                            addToBot(new CrushJoints_Vary_Action(m, _inst.magicNumber));
                            return SpireReturn.Return(null);
                        case CrushJoints_Cripple:
                            addToBot(new DamageAction(m, new DamageInfo(p, _inst.damage, _inst.damageTypeForTurn),
                                    AbstractGameAction.AttackEffect.BLUNT_HEAVY));
                            addToBot(new CrushJoints_Cripple_Action(m, p, _inst.magicNumber));
                            return SpireReturn.Return(null);
                    }
            }
            return SpireReturn.Continue();
