...
public class DefendPEvo extends Evolution {
    ...
    public static int branchID(AbstractCard card) {
        ...
        switch (chosenBranch) {
            case 1:
                return Defence_P_Foresee;
            case 2:
                return Defence_P_Holy;
            default:
                return 0;
        }
    }
    ...
    public static List<UpgradeBranch> branches(AbstractCard card) {
        return new ArrayList<UpgradeBranch>() {{
            ...
            add(() -> {
                upgradeEvolvedTexts(card, Defence_P_Foresee);
                setBlock(card, 6);
                setMagic(card, 3);
            });
            add(() -> {
                upgradeEvolvedTexts(card, Defence_P_Holy);
                setBlock(card, 6);
                setMagic(card, 2);
            });
        }};
    ...
    @SpirePatch(clz = Defend_Watcher.class, method = SpirePatch.CONSTRUCTOR)
    public static class Evolve {
        @SpireRawPatch
        public static void Raw(CtBehavior ctMethodToPatch) throws Exception {
            if (!manipulateMethods(DefendPEvo.class, ctMethodToPatch))
                Nature.Log("Defend Purple is not evolvable.");
        }
    }
